...
import { getModulesByTurma, getUserProgress } from '../lib/api'
...
export default function Dashboard() {
    ...
    async function load() {
      ...
      try {
        const [mods, prog] = await Promise.all([
          profile.turma_id ? getModulesByTurma(profile.turma_id) : Promise.resolve([]),
  ...
  const completedCount = modules.reduce((acc, m) => m.is_locked ? acc : acc + (m.lessons || []).filter(l => completedSet.has(l.id)).length, 0)
  const progressPercent = totalLessons > 0 ? Math.round((completedCount / totalLessons) * 100) : 0
  const firstName = profile?.full_name?.split(' ')[0] || 'Atleta'
  const turmaName = profile?.turmas?.name || 'Sua turma'
    ...
    <div className="min-h-screen bg-bg-dark">
      <div className="px-5 pt-6 pb-7 border-b border-bahia-blue/[0.12]"
        ...
        <div className="flex justify-between items-start mb-5">
          <div>
            <p className="text-bahia-gold text-xs font-semibold uppercase tracking-wider mb-1">{isAdmin ? 'Administrador' : `Turma ${turmaName}`}</p>
            <h1 className="text-white text-[22px] font-bold">Olá, {firstName} 👋</h1>
          </div>
          <div className="flex items-center gap-3">
            {isAdmin ? (
              <button onClick={() => navigate('/admin')} className="bg-bahia-gold text-bahia-blue-dark text-xs font-bold rounded-xl px-3.5 py-2 hover:brightness-110 transition-all">Painel Admin</button>
            ) : (
      ...
      {isAdmin && (
        <div className="p-5">
          <button onClick={() => navigate('/admin')} className="card card-active w-full text-left flex items-center gap-3.5 cursor-pointer">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 text-xl bg-bahia-gold/[0.15]">🛠️</div>
            <div className="flex-1">
              <p className="text-white text-[15px] font-semibold">Painel de administração</p>
              <p className="text-bahia-blue-light/40 text-xs">Matriculados, progresso e exportar CSV</p>
            </div>
          </button>
        </div>
      )}

      <div className="p-5" style={isAdmin ? { display: 'none' } : {}}>
            ...
            return (
              <button key={mod.id} onClick={() => !mod.is_locked && navigate(`/module/${mod.id}`)}
                ...
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 text-sm font-bold
                  ...
                  {mod.is_locked ? '🔒' : allDone ? '✓' : mod.sort_order}
                </div>
